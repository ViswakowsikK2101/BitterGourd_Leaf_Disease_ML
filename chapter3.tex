...
The generated directory also contains normalized per-class confusion matrices, a CNN learning curve, a score-comparison figure and a training-time figure. Review false-negative rates for each diseased class, not merely the aggregate score; a high healthy-class recall cannot compensate for missed disease.

\begin{figure}[htbp]\centering
\IfFileExists{results/model_comparison.png}{\includegraphics[width=.95\textwidth]{results/model_comparison.png}}{\fbox{Generated model-comparison screenshot will appear here.}}
\caption{Accuracy, precision, recall, macro-F1 and macro ROC-AUC across the benchmark models.}\label{fig:modelcomparison}
\end{figure}

\begin{figure}[htbp]\centering
\IfFileExists{results/training_time.png}{\includegraphics[width=.85\textwidth]{results/training_time.png}}{\fbox{Generated training-time screenshot will appear here.}}
\caption{Training-time comparison on the same training split.}\label{fig:trainingtime}
\end{figure}
